            ...
            if self.department == "Production":
                ...
                try:
                    if col == 0:
                        ...
                        return int(self.data(self.index(row,col-1),QtCore.Qt.DisplayRole)) + int(self.data(self.index(row,col-2), QtCore.Qt.DisplayRole))
                        ...
                        #Daily Earned Rs
                        return self.employees[row].dayspresent*self.employees[row].salary if self.employees[row].salary_interval == "Daily" else 0
    ...
    def setData(self, index, value, role):
        ...
        if role == QtCore.Qt.EditRole:
            ...
            if self.department == "Production":
                ...
                self.employees[row].setAttendanceDict(self.month, self.year)
                self.employees[row].setAttendanceHalf(self.month, self.half)
                self.dataChanged.emit(index,index) # This recalls the data function on all cells bounded by the rectangle (topleft,bottomright) which is (index,index) but since our data function has no way to see the updated database, the tableview doesnt update...comment might be outdated... lol
            return True
        else:
            return False
